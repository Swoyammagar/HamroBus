...
import { useBus, useRoute, type BusRecord } from '../../context/domains';
...
const Buses: React.FC = () => {
  ...
  const { approvedDrivers, fetchApprovedDrivers } = useAdminDrivers();
  ...
  }, [fetchAllBuses, fetchApprovedDrivers]);
  ...
  const columns: TableColumn<Bus>[] = [
    ...
    { key: 'capacity', header: 'Capacity', flex: 0.8 },
    {
      ...
      flex: 1,
      render: (item) => (
        <StatusBadge label={item.status || 'unknown'} variant={getStatusVariant(item.status || '')} />
      ),
    },
    {
      key: 'driver',
  ...
  const driverOptions: PickerOption[] = approvedDrivers.map((d) => ({
  ...
  const handleSaveEdit = async () => {
    ...
    if (originalRouteId !== newRouteId) {
      // Close edit modal FIRST so confirm card renders on top
      setModalVisible(false);
    ...
    <View style={styles.container}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.form}>
            ...
            <Input
              ...
              onChangeText={(t) => setAddFields((s) => ({ ...s, capacity: Number(t) || 0 }))}
            />
            <Picker
              label="Status"
            ...
            <Picker
              ...
              onSelect={(val) => setAddFields((s) => ({ ...s, assignedDriverId: String(val) || undefined }))}
            ...
            <Picker
              ...
              onSelect={(val) => setAddFields((s) => ({ ...s, assignedRouteId: String(val) || undefined }))}
            ...
            <View style={styles.formActions}>
              <Button onPress={resetAddForm} variant="secondary" disabled={isSubmitting}>
                Reset
              </Button>
              <Button onPress={handleAddBus} variant="primary" disabled={isSubmitting}>
      ...
      <Modal
        ...
        footer={
          <>
            <Button
              onPress={() => { setModalVisible(false); setEditingBus(null); }}
              variant="secondary"
              disabled={isSubmitting}
            >
              Cancel
            </Button>
            <Button onPress={handleSaveEdit} variant="primary" disabled={isSubmitting}>
      ...
      >
        ...
        <Input
          ...
          onChangeText={(t) => setEditFields((s) => ({ ...s, capacity: Number(t) || 0 }))}
        />
        <Picker
          label="Status"
        ...
        <Picker
          ...
          onSelect={(val) => setEditFields((s) => ({ ...s, assignedDriverId: String(val) || '' }))}
          options={driverOptions}
        ...
        <Picker
          ...
          onSelect={(val) => setEditFields((s) => ({ ...s, assignedRouteId: String(val) || '' }))}
          options={routeOptions}
        ...
        <View style={styles.confirmOverlay}>
          <View style={styles.confirmCard}>
            ...
            <Text style={styles.confirmSub}>
              This action cannot be undone. The bus will be permanently removed.
            </Text>
      ...
      {confirmRouteChange && (
        <View style={styles.confirmOverlay}>
          <View style={styles.confirmCard}>
            ...
            <Text style={styles.confirmSub}>
              Assigned schedules need to be changed manually after this update.
            </Text>
            <View style={styles.confirmActions}>
              <TouchableOpacity
                onPress={() => {
                  ...
                  // Re-open edit modal so user can make changes
                  setModalVisible(true);
                }}
                style={styles.confirmCancel}
              >
              ...
              <TouchableOpacity
                onPress={async () => {
                  // Capture payload into local var before clearing state
                  const payload = pendingEditPayload;
                  setConfirmRouteChange(false);
                  setPendingEditPayload(null);
                  if (payload) {
                    await submitBusEdit(payload);
                  }
                }}
