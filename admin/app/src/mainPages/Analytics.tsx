...
const Analytics = () => {
  ...
  const getBusNumber = (review: AdminReviewItem) => {
    ...
    if (booking.busId && typeof booking.busId !== 'string') {
      return booking.busId.busNumber || '-';
    }
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter((review) => {
      ...
      const driverName = getDriverName(driver).toLowerCase();
      const busNumber = getBusNumber(review).toLowerCase();
      return driverName.includes(q) || busNumber.includes(q);
    });
  }, [reviewQuery, reviews]);

  const summaryCards = useMemo(() => {
    const topDriver = leaderboard[0];
    const topDriverName = topDriver ? `${topDriver.firstName || ''} ${topDriver.lastName || ''}`.trim() || 'Unknown' : '—';

    return [
      {
        label: 'Reviews Loaded',
        value: String(reviews.length),
        helper: 'Across all drivers',
        tone: 'teal',
      },
      {
        label: 'Drivers Ranked',
        value: String(leaderboardTotal),
        helper: 'Leaderboard entries',
        tone: 'blue',
      },
      {
        label: 'Global Avg Rating',
        value: globalAverage.toFixed(2),
        helper: 'Overall review average',
        tone: 'amber',
      },
      {
        label: 'Top Driver',
        value: topDriver ? `#${topDriver.rank}` : '—',
        helper: topDriverName,
        tone: 'slate',
      },
    ];
  ...
  return (
    <View style={styles.container}>
      ...
      <Tabs
        ...
        activeTab={activeTab}
        onTabChange={(key) => setActiveTab(key as 'reviews' | 'leaderboard')}
                ...
                return (
                  <Card key={review._id} padding="sm" style={reviewCardStyle}>
                    ...
                    <View style={styles.reviewFooter}>
                      <Text style={styles.reviewFooterText}>Booking {review.bookingId && typeof review.bookingId !== 'string' ? review.bookingId.bookingCode || '-' : '-'}</Text>
          ...
          {leaderboardLoading ? (
            ...
            <ScrollView style={styles.leaderboardScroll} contentContainerStyle={styles.cardGrid}>
              {leaderboard.map((row, index) => {
                ...
                const leaderCardStyle = highlight
                  ? { ...styles.leaderCard, ...styles.leaderCardTop }
                  : styles.leaderCard;

                return (
                  <Card key={row.driverId} padding="md" style={leaderCardStyle}>
                    ...
                    <View style={styles.leaderBottomRow}>
                      ...
                      <Text style={styles.leaderBottomMeta}>
                        {row.validationStatus || 'unknown'} • {typeof row.isActive === 'boolean' ? (row.isActive ? 'active' : 'inactive') : 'status unknown'}
                      </Text>
                    </View>
                  </Card>
                );
